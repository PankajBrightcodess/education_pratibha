	    	...
	    	if($password==$con_password){
	    	    ...
	    	    $sql=mysqli_query($conn,$query);
	    	     $last_id = $conn->insert_id;
	    	      $query1= "INSERT INTO `wallet`(`user_id`,`type`,`amount`,`name`,`email`,`date`) VALUES ('$last_id','field_executive','0','$name','$email','$added_on')";
...
if(isset($_POST['del_result_admin'])){
  ...
  if(isset($_POST['del_material'])){
   ...
   if(isset($_POST['student_reg'])){
	    ...
	    $num=mysqli_num_rows($run);
	    if($num==0){
	    		$query="INSERT INTO `student`(`ref_id`,`name`,`mobile`,`email`,`school_name`,`fathername`,`bankname`,`bankaccount`,`ifsc`,`ac_qualify`,`executive_id`,`password`,`dob`,`address`,`added_on`) VALUES ('$refid','$name','$mobile','$email','$school_name','$fathername','$bankname','$bankaccount','$ifsc','$ac_qualify','$executive_id','$password','$dob','$address',
	    			'$added_on')";
	    	    $sql=mysqli_query($conn,$query);
	    	    $last_id = $conn->insert_id;
			...
			header("location:$_SERVER[HTTP_REFERER]"); 
		     ...
		     echo '<script>alert("Enter Another Email!!");</script>';
			 header("location:$_SERVER[HTTP_REFERER]"); 
	    }
	}else{
		    echo '<script>alert("Enter Another Email!!");</script>';
